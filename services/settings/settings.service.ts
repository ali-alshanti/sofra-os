...
export async function getRestaurantSettings(
  ...
  const [restaurantRes, settingsRes, hoursRes] = await Promise.all([
    ...
    supabase
      ...
      .select("currency, tax_rate, tax_label, language, theme, date_format, notification_preferences")
  ...
  const r   = restaurantRes.data as RestaurantRow;
  ...
  const notifications: NotificationSettings = {
    ...
    newReservation: rawNotif["newReservation"] ?? DEFAULT_NOTIFICATION_PREFS.newReservation,
    staffAlert:     rawNotif["staffAlert"]     ?? DEFAULT_NOTIFICATION_PREFS.staffAlert,
    dailySummary:   rawNotif["dailySummary"]   ?? DEFAULT_NOTIFICATION_PREFS.dailySummary,
  };
...
export async function getIntegrations(_restaurantId: string): Promise<Integration[]> {
  // No integrations table exists yet. Return the static catalogue.
